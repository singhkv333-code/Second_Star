...
tool("screen_fundamentals",
     ...
     "RANKING — YOU own the sort: for ANY ordered ask ('best', 'top', "
     "'strongest', 'highest ROE', 'cheapest', 'fastest-growing', 'largest') set "
     "sort_by to the field that fits the intent. The tool does NOT pick a metric "
     "for you — there is no hidden metric-priority default. If you omit sort_by "
     "the screen falls back to a NEUTRAL market-cap (size) ordering only, which "
     "is almost never what 'best' means, so decide the ranking yourself. "
     "Single-dimension ask → sort on that dimension (cheapest → {field:pe,dir:"
     "asc}; highest quality → {field:roe,dir:desc}; fastest-growing → {field:"
     "revenue_growth,dir:desc}). BROAD/judgment 'best' ask → put several "
     "relevant metrics in play (see BROAD/JUDGMENT above), sort_by the dimension "
     "you judge most central, and set presentation='analysis' to defend the "
     "ranking. Omit sort_by ONLY when the user expressed no ordering at all (a "
     "pure filtered list). You are smart enough to choose — do it, don't defer "
     "to a default.\n\n"
