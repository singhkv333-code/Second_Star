"""Map Pivot sc_ids → NSE tickers using the configured LLM, verify via yfinance.
...
have to bridge ``sc_id='RI'`` → ``RELIANCE``. Approach:
  ...
  2. Batch the rest into groups of ~30 and ask the LLM to emit strict JSON
     ``[{"sc_id":..., "nse_symbol":"..."|null}]``.
  3. For each non-null answer, verify by issuing a 5-day ``yf.download`` —
     a missing/empty result means the LLM hallucinated, mark NULL.
...
_BATCH_SIZE = 30                       # safe per-call json payload size
...
async def map_and_persist(
    ...
    *,
    ...
    """
    if not sc_ids:
        return {"total": 0, "already_mapped": 0, "asked_llm": 0,
    ...
    if not targets:
        return {"total": len(sc_ids), "already_mapped": already, "asked_llm": 0,
                "verified": 0, "rejected": 0, "results": []}

    proposals: dict[str, str | None] = {}
    for batch in _chunks(targets, _BATCH_SIZE):
        batch_proposals = await _ask_llm(batch)
    ...
    rejected: dict[str, str] = {sc: "llm_null"
...
async def _ask_llm(batch: list[tuple[str, dict]]) -> dict[str, str | None]:
    """Single LLM call for a batch of (sc_id, row). Returns sc_id → ticker map."""
    payload_in = [
    ...
    try:
        ...
        resp = await client.complete(
            ...
            # JSON-object mode rejects top-level arrays — we parse the
            # raw content manually instead.
        )
    except Exception as e:
        logger.warning("LLM symbol-mapping call failed: %s", e)
        return {sc: None for sc, _ in batch}

    content = (resp.content or "").strip()
    parsed = _coerce_json_array(content)
    if not isinstance(parsed, list):
        logger.warning("LLM returned non-list mapping: %r", content[:200])
        return {sc: None for sc, _ in batch}
...
def _coerce_json_array(text: str):
    """The LLM sometimes wraps the array in markdown fences — peel them off."""
    if not text:
        return None
