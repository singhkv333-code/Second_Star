...
from backend.services.portfolio_cache import (
    cache_aside, get_holdings_cached, performance_cache_key,
)
...
def _compute_performance(
    user_id: int, token: str, period: _PeriodLiteral,
) -> dict:
...
def get_performance(
    ...
    data = cache_aside(
        performance_cache_key(user_id, period),
        lambda: _compute_performance(user_id, token, period),
    )
