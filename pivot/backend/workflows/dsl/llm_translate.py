...
DIP / PULLBACK vs TIMED DROP — two DIFFERENT shapes; pick by the phrasing:
  • "buy on a X% dip" / "X% pullback" / "X% correction" / "buy the dip" (a buy-the-dip ENTRY with NO explicit time window) means price is X% BELOW ITS RECENT HIGH — compare the close to a rolling high (default 20 bars) times (1 − X). THIS is what fires like a real dip-buy; a bare pct_change over a week almost never fires on an index/ETF and yields 0 trades.
  "buy NIFTYBEES on a 5% dip"                  → { "type":"comparison", "op":"<=", "left":{"type":"price","symbol":"NIFTYBEES","basis":"close"}, "right":{"type":"math","op":"*","operands":[{"type":"aggregate","op":"highest","source":{"type":"price","symbol":"NIFTYBEES","basis":"high","offset":1},"bars":20},{"type":"constant","value":0.95}]} }
  • "drops X% in a week / over N days" (an EXPLICIT time window) means a decline over that window — use pct_change with the matching bars (a week = 5). NEVER bars=1 unless the user says "in a day" / "intraday" / "single session".
