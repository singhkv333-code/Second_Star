...
- **Weighted BASKET buy-and-hold ("backtest this basket, hold to end, no rebalancing", or a list of names+weights).** This is the MOST-MISHANDLED shape — get it right: emit ONE `backtest_workflow` with a **one-time** `trigger.schedule` (`run_at`) + exactly **ONE** `action.allocate_basket` whose `legs[]` carry every constituent as `{symbol, exchange, weight, side:"long"}`, and **NO exit** (hold to end). Set `benchmark_symbol` (default `NIFTYBEES`) so the comparison isn't one leg.
  - **NEVER** express a basket as one `action.place_order` per name — the engine then anchors the whole result on the FIRST order's ticker, throws the weights away, and reports one stock's scheduled-buy as if it were the basket (the "8-name oil basket showed up as *Bharat Petroleum · Scheduled buy strategy*" bug). One `allocate_basket` with weighted legs is the ONLY correct shape.
  - **NEVER collapse a multi-name basket to a single symbol** or `backtest_dsl_tree` (that engine is single-symbol). If the user gave weights, pass them verbatim in `legs`; if they gave only names, equal-weight and say so.
  - *Canonical:* "Backtest this basket buy-and-hold 3y, no rebalance: GAIL 16%, ONGC 18%, NTPC 19%, POWERGRID 20%, …" → `backtest_workflow(name="…basket buy & hold", period="3y", benchmark_symbol="NIFTYBEES", steps=[{trigger.schedule run_at:"<window start>"}, {action.allocate_basket total_inr:1000000, legs:[{symbol:"GAIL",exchange:"NSE",weight:0.16,side:"long"}, …]}])`.
...
## "Buy on an X% dip" — a dip is a pullback from the RECENT HIGH, not a one-day drop
When the user says "buy on a 5% dip" (or "buy the dip", "on a X% pullback/correction"), they mean **price has fallen X% below its recent high** — a pullback you can actually catch. Express it in `backtest_dsl_tree` as `close ≤ (1 − X/100) × rolling_high(N)` using a rolling recent high (default **N = 20 bars**) — e.g. for a 5% dip: `condition="close <= rolling_high(20) * 0.95"` (or the natural-language equivalent "buy when price is 5% below its 20-day high"). Then put the take-profit in `exit_condition` ("X% profit" = unrealised P&L ≥ X% of the dip entry).
