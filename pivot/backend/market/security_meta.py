...
_US_META: dict[str, tuple[str, str]] = {
    ...
    "GD": ("General Dynamics", "gd.com"),
    "CB": ("Chubb", "chubb.com"),
    "ALL": ("Allstate", "allstate.com"),
    "LMT": ("Lockheed Martin", "lockheedmartin.com"),
    "RTX": ("RTX", "rtx.com"),
    "NOC": ("Northrop Grumman", "northropgrumman.com"),
    "UBS": ("UBS", "ubs.com"),
    "TRV": ("Travelers", "travelers.com"),
    "PGR": ("Progressive", "progressive.com"),
    "SNAP": ("Snap", "snap.com"),
    "PINS": ("Pinterest", "pinterest.com"),
    # popular US ETFs
...
# Display names for the handful of NSE tickers whose symbol is not a readable
# name (RECLTD -> REC). Carried here on 2026-09-05 from the retired
# `plain_copy.py`, which was the only reason this module imported an 833-line
# file of prose to use one ten-line function from it.
...
def stock_name(symbol: Optional[str]) -> Optional[str]:
    """Plain display name for a tradeable equity symbol (RECLTD.NS -> REC)."""
    if not symbol:
        return None
    base = str(symbol).strip().split("(")[0].strip().split(".")[0].strip()
    if not base:
        return None
...
def _load_in_etf_symbols() -> set[str]:
    ...
    try:
        from backend.market.etf_catalog import load_catalog
        syms: set[str] = set()
