...
- ✅ **2.4 DONE (2026-06-01)** **Multi-position portfolio state.** The tree engine is single-position, so this is
  a NEW multi-symbol engine `backend/services/backtest/portfolio/` (also lands the 2.1 momentum + dollar-neutral
  L/S that were price-gated; OHLCV = yfinance). ✅ causal core — `momentum_scores` (12-1, data ≤ t),
  `target_weights` (**max names** + **gross budget** + **per-sector name cap** enforced by construction;
  long-only or dollar-neutral **L/S**), `simulate_portfolio` (drifting weights, one-bar-lag rebalance, turnover
  cost). ✅ **sector caps** via a network-free curated `symbol→sector` map (`sector_universe.symbol_sector_map`);
  confirmed binding (1-name/sector on a steel-heavy basket: 31.9% → 60.5%). ✅ REST `/api/backtest/portfolio/run`
  + chat tool `backtest_portfolio` (routing solid for clear top-N/L-S phrasings; one borderline phrasing
  declines-to-guess rather than mis-route). Live (15 large-caps, monthly, 5y): long-only momentum +28.5%
  (unproven, PSR 0.83); L/S net≈0 (dollar-neutral, no_edge). 11 tests incl. two no-look-ahead proofs. **[M]**
- ✅ **2.5 DONE (2026-06-01)** **Reference-strategy acceptance tests** —
  `tests/test_reference_strategies.py`, one canonical strategy per class, each asserted through the rigor
  ladder (shared `_assert_rigor_ladder`: forward_stats keys + monte_carlo + sub_periods + verdict ∈ the
  4-way ladder): (1) single-symbol RSI mean-reversion on a SYNTHETIC fetcher (no network — always-on guard);
  (2) pairs Engle-Granger spread; (3) momentum portfolio long-only (gross ≤ 1); (4) momentum L/S (net ≈ 0);
  (5) fundamental factor screen `decile(neutralize(roe))==10 AND D/E<0.6`; (6) Johansen basket. 6/6 pass live;
  the 5 live ones skip cleanly if yfinance/DB are down. Regression guard for the engines + look-ahead + rigor
  wiring. **[M]**

**→ PHASE 2 COMPLETE (2.1 transforms · 2.2 sizing · 2.3 pairs/baskets · 2.4 portfolio · 2.5 acceptance).**
...
4. ✅ **DONE — Phase 1.4 — walk-forward + no-skill permutation** (`validation/walkforward.py`). Built the
   **warmup-aware engine-rerun adapter** (each eval window padded with `warmup` bars so indicators stay warm —
   no clip-to-fold corruption). `permutation_test` (shuffle-returns null → p-value; discrimination validated)
   + `walk_forward` (OOS folds + consistency) + `deep_validate_engine2b`; REST `POST /api/backtest/dsl/validate`
   (opt-in — N re-runs). 7 tests. Chat tool for it still pending. (The adapter also unblocks cost-sensitivity
   sweeps.)
5. **Phase 1.5 — CPCV→PBO** (needs a parameter grid → couples to P2). *(Trust verdict 1.8 + FE card 1.9 already
   shipped.)*
