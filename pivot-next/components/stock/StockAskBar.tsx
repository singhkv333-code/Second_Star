 ...
 * answer here obeys the same contract: real tool values, the relay tagged
 * when it isn't Kite, and the analysis-not-advice close.
 *
...
const TOOL_WORD: Record<string, string> = {
  ...
  analyze_stock: "Pulling the full picture",
  compare_stocks: "Comparing",
...
export function StockAskBar({
  ...
  // rather than carrying the last one's subject into this one — the backend
  // keys its Redis state on this id, and "it" must not mean the previous
  // company.
    ...
    <>
      ...
      >
        ...
        <form className="stock-ask-pill" onSubmit={submit}>
          ...
          <textarea
            ...
            aria-label={`Ask Pivot about ${sym}`}
        ...
        {expanded && !turns.length ? (
          <div className="stock-ask-hint" aria-hidden>
            Pivot answers with this company&apos;s own numbers. Not financial advice.
          </div>
        ) : null}
      </div>
      ...
      <style>{`
        ...
        .stock-ask-mark {
          ...
          -webkit-mask: url("/charto-mark.png") center / contain no-repeat;
