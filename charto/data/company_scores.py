...
Four models, all read out of the same MC statement grids the statements panel
...
_GRIDS: "Callable[[str, str, str], dict | None]" = lambda *a: None


def bind(reader) -> None:
    """Install the statement reader.

    The scorer is Pivot's, unchanged apart from this seam: it asks for a
    statement grid by (symbol, statement, basis) and does arithmetic. Charto
    hands it grids out of its own store rather than opening Pivot's database,
    so the company page keeps running when Pivot is not.
    """
    global _GRIDS
    _GRIDS = reader
...
# preference order.
#
# The unit suffix is not to be trusted either: Infosys and ITC file
# "Price/BV (%)" carrying 5.45 and 4.97 — multiples, not percentages (Infosys
# at ~Rs 1,250 over a filed BVPS of Rs 229 is 5.45x). Reading the suffix as a
# unit would divide two large caps' Altman Z by a hundred; it is treated as
# part of the label and nothing more.
...
def _altman_z(bs: dict, pl: dict, rt: dict, period: str) -> dict:
    ...
    pb = _pick(rt, period, "Price/BV (X)", "Price/BV (%)",
                 "Price To Book Value (X)", "Price To Book Value (%)")
...
def compute_scores(symbol: str, *, basis: str = "consolidated") -> dict:
    ...
    # One period for every score — and the newest period that BOTH sheets
    # reach, not simply the newest the balance sheet reaches.
    #
    # The two grids do not end together. AMBER's balance sheet runs to Mar 26
    # while its P&L stops at Mar 25, so leading with the balance sheet pinned
    # Mar 26 and then every P&L lookup came back empty — Altman, Ohlson and
    # DuPont all fell out for a company that has the data to support them.
    # Leading with the intersection costs one year in that case and buys three
    # scores. Where the sheets never overlap the balance sheet still leads, so
    # the distress models keep the sheet they are defined on.
    periods = bs_grid.get("periods") or []
    if not periods:
        return {"available": False, "symbol": sym, "reason": "no filed statements"}
    pl_periods = set((grids["profit_loss"] or {}).get("periods") or [])
    shared = [p for p in periods if p in pl_periods]
    usable = shared or periods
    period = usable[0]
    prior = usable[1] if len(usable) > 1 else None
